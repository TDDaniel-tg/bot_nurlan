...
{result['text'][:2000]}{'...' if len(result['text']) > 2000 else ''}"""
                ...
                # Кнопка для экспорта в Excel (только если есть таблицы)
            ...
            try:
                ...
                if not table_data:
                    await query.edit_message_text(
                        "❌ Таблицы не найдены в PDF.\n\n"
                        "Возможные причины:\n"
                        "• PDF содержит только текст без таблиц\n" 
                        "• Таблицы в виде изображений (OCR их не распознал)\n"

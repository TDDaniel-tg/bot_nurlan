                ...
                messages=[
                    {
                        ...
                        "content": [
                            ...
                            {
                                "type": "text",
                                "text": """Найди и точно извлеки все таблицы из этого изображения.
                                КРИТИЧЕСКИ ВАЖНО: сохраняй все имена, фамилии и отчества ТОЧНО как написано в документе, без изменений, исправлений или переводов.
                                
                                Для каждой найденной записи в таблице верни JSON объект со следующими полями:
                                - fio: ФИО (точно как написано в документе, без изменений)
                                - birth_date: дата рождения (в исходном формате)
                                - position: должность (точно как в документе)
                                - harmful_factors: факторы вредности (точно как в документе)
                                - other: любая другая важная информация (точно как в документе)
                                
                                ПРАВИЛА:
                                1. НЕ исправляй орфографию в именах
                                2. НЕ переводи имена на другие языки
                                3. НЕ меняй регистр букв в именах
                                4. Сохраняй все символы и пробелы как есть
                                5. Если не уверен в букве - используй наиболее похожую

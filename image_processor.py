                ...
                messages=[
                    {
                        ...
                        "content": [
                            ...
                            {
                                "type": "text",
                                "text": """Извлеки весь текст из этого изображения ТОЧНО как написано.
                                КРИТИЧЕСКИ ВАЖНО: сохраняй все имена, фамилии и отчества ТОЧНО без изменений.
                                
                                Сохрани структуру и форматирование. 
                                Если есть таблицы, представь их в структурированном виде.
                                
                                ПРАВИЛА:
                                1. НЕ исправляй орфографию в именах людей
                                2. НЕ переводи имена 
                                3. Сохраняй все символы и регистр как есть
                                4. Если не уверен в букве - используй наиболее похожую
                ...
                messages=[
                    {
                        ...
                        "content": [
                            ...
                            {
                                "type": "text",
                                "text": """Проанализируй этот документ и извлеки всю структурированную информацию.
                                КРИТИЧЕСКИ ВАЖНО: сохраняй все имена, названия, адреса и числа ТОЧНО как написано без изменений.
                                
                                Определи тип документа и извлеки соответствующие данные:
                                
                                ЕСЛИ ЭТО ДОКУМЕНТ ОБ ОРГАНИЗАЦИИ (карта партнера, реквизиты, справка):
                                Создай объекты со следующими полями:
                                - organization_name: название организации (точно как написано)
                                - inn: ИНН (если есть)
                                - kpp: КПП (если есть) 
                                - address: адрес (полностью как написано)
                                - director: ФИО директора (точно как написано)
                                - phone: телефон (как написано)
                                - email: email (как написано)
                                - license: лицензия (если есть)
                                - other: любая другая важная информация
                                
                                ЕСЛИ ЭТО ТАБЛИЦА С ЛЮДЬМИ (медосмотры, списки сотрудников):
                                Создай объекты со следующими полями:
                                - fio: ФИО (точно как написано в документе)
                                - birth_date: дата рождения (в исходном формате)
                                - position: должность (точно как в документе)
                                - department: отдел/подразделение (если есть)
                                - other: любая другая информация
                                
                                ПРАВИЛА:
                                1. НЕ исправляй орфографию в именах и названиях
                                2. НЕ переводи на другие языки
                                3. НЕ меняй регистр букв
                                4. Сохраняй все символы, пробелы и знаки как есть
                                5. Если не уверен в символе - используй наиболее похожий
                                6. Извлекай каждую строку данных как отдельный объект
                                
                                Верни результат в формате JSON массива объектов.
                                Если структурированных данных нет, верни пустой массив []."""
                            }
                        ]
                    }
                ],
